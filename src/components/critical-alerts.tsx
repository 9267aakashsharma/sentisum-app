...
type AlertSeverity = (typeof severity)[number];
type AlertSource = (typeof source)[number];

function getHexColorBySeverity(severity: AlertSeverity) {
  switch (severity) {
    case "info":
      return "#444ce7";
    case "warning":
...
const getHeroUIColorBySeverity = (severity: AlertSeverity) =>
  ...
  return (
    <div className="shadow-md border bg-white border-gray-200 rounded-xl p-4 relative">
      ...
      <div className="flex items-center gap-2 justify-between">
        <p className="font-semibold text-lg">Critical Alerts</p>
      ...
      <AlertModal alert={selectedAlert} onClose={handleAlertModalClose} />
    </div>
  );
};
...
const SeverityFilter = ({
  ...
  const handleSeverityChange = (severity: string) => {
    const updatedSeverities = (severity.split(",") as AlertSeverity[]).filter(
      (s) => s.length
    );
    ...
    <div className="flex-1 flex justify-start items-center">
      ...
      >
        {Object.keys(severityOptions).map((key) => (
          <SelectItem key={key}>
            {severityOptions[key as AlertSeverity]}
          </SelectItem>
        ))}
      </Select>
    </div>
...
const SourceFilter = ({
  ...
  setSelectedSources: (sources: AlertSource[]) => void;
}) => {
  const handleSourceChange = (source: string) => {
    const updatedSources = (source.split(",") as AlertSource[]).filter(
      (s) => s.length
    );
    ...
    <div className="flex-1 flex justify-end items-center">
      ...
      >
        {Object.keys(sourceOptions).map((key) => (
          <SelectItem key={key}>{sourceOptions[key as AlertSource]}</SelectItem>
        ))}
      </Select>
    </div>
  );
...
const AlertList = ({
  ...
  const filteredAlerts = alerts.filter((alert) => {
    ...
    const matchesSource =
      ...
      selectedSources.includes(alert.source as AlertSource);
    const matchesSearch =
          ...
          title={
            <div className="flex items-center justify-between gap-2">
              ...
              <Chip
                ...
                color={getHeroUIColorBySeverity(alert.severity)}
              >
                {alert.severityText ?? alert.severity}
              </Chip>
            </div>
          }
          classNames={{
          ...
          <div className="-mt-2.5">
            ...
            <div className="inline-flex items-center gap-2">
              <p className="text-tiny">
                {format(new Date(alert.timestamp), "E dd, hh:mm aaa")}
              </p>
            </div>
          </div>
        </AccordionItem>
        ...
        {(onClose) => (
          <>
            <ModalHeader className="flex flex-col gap-1 border-b-1 border-neutral-100 mx-2 pt-4 pb-3 px-2">
              <h4 className="font-text">{alert.title}</h4>
              <Chip
                size="sm"
                variant="flat"
                radius="sm"
            ...
            <ModalBody>
              ...
              <div>
                <AreaDataChart
                  ...
                  fillColor={getHexColorBySeverity(alert.severity)}
                  strokeColor={getHexColorBySeverity(alert.severity)}
                />
              </div>
            </ModalBody>
